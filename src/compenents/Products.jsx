...
function Products() {
  ...
  {/* Map through products array and create a product card for each item */}
  {productsData.map(product => (
    <div key={product.id} className="max-w-sm rounded overflow-hidden shadow-lg">
      ...
      <div className="px-6 py-4">
        <button onClick={() => dispatch(addcart({id:product.id,title:product.title,price:product.price,description:product.description,thumbnail:product.thumbnail}))} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          Add to Cart
        </button>
      </div>
    </div>
